...
import { Navigation, Shield, MapPin, AlertTriangle, RotateCcw, Brain, ArrowRight } from "lucide-react";

type Phase = "idle" | "analyzing" | "loading" | "preview" | "walking" | "result";

function IdleState({ onStart, error }: { onStart: () => void; error: string | null }) {
  return (
    <>
      {error && (
      ...
      {!error && (
        <div className="text-center py-16 space-y-4">
          ...
          <p className="text-muted-foreground max-w-md mx-auto">
            You don't have to guess which streets feel safer—Goose will check lighting and incidents for you.
          </p>
...
function RoutePreview({ route, onStartWalk, onReset }: { route: SafeRouteResponse; onStartWalk: () => void; onReset: () => void }) {
  return (
    <>
      {/* Goose summary card */}
      <div className="bg-card rounded-xl border border-primary/30 p-5 space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          {route.safetyscore >= 8 && <span className="ml-2 text-accent">· Confidence: High</span>}
        </p>
        <p className="text-xs text-muted-foreground">
          Goose favors well-lit streets and places where people are around, and avoids recent incident hotspots.
        </p>
        <GooseExplainDrawer safetyScore={route.safetyscore} />
      </div>
...
export default function MapPage() {
    ...
    return (
      <div className="container py-10 space-y-4">
        ...
        <p className="text-xs text-muted-foreground text-center">
          You're not walking alone—Goose will keep you on better-lit, more active streets.
        </p>
        <ARSimulation
  ...
  return (
    <div className="container py-10 space-y-6">
      ...
      {phase === "preview" && route && (
        <RoutePreview route={route} onStartWalk={() => setPhase("walking")} onReset={reset} />
      )}

      {phase === "idle" && (
        <IdleState onStart={startAnalysis} error={error} />
      )}
    </div>
  );
}
